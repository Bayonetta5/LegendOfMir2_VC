...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		m_bCurrMtn		= bMtn;
		m_bCurrDir		= BYTE(bDir);
		m_bMoveDir		= BYTE(bDir);
		m_wPosX			= wPosX;
		m_wPosY			= wPosY;
...
VOID CActor::OnUserName(LPPACKETMSG lpPacketMsg)
{
	...
	m_dwNameClr = BYTE(GetUserNameColor(BYTE(lpPacketMsg->stDefMsg.wParam)));
}
...
VOID CActor::OnLighting(LPPACKETMSG lpPacketMsg)
{
	...
	MESSAGEBODYWL stMsgBodyWl;
	fnDecode6BitBuf(lpPacketMsg->szEncodeData, (char*)&stMsgBodyWl, sizeof(MESSAGEBODYWL));
	wTargetX = BYTE(stMsgBodyWl.lParam1);
	wTargetY = BYTE(stMsgBodyWl.lParam2);
...
BOOL CActor::LoadEffect(CImageHandler* pxImgHandler, WORD wEffectNum, BYTE bDir)
{
	...
	if ( pstEffect )
	{
		...
		for ( INT nCnt = m_dwFstEffectFrame; nCnt < INT(m_dwEndEffectFrame); nCnt++ )
		{
		...
		if( SUCCEEDED(g_xMainWnd.Get3DDevice()->BeginScene()) )
		{
			...
			if ( m_bUseEffect || m_bUseSwordEffect )
			{
				...
				if ( bIndexSetted )
				{
					...
					LPDIRECTDRAWSURFACE7 lpddsTextr = g_xGameProc.m_xImage.GetTextrImg(WORD(wFileType), WORD(m_bEffectImgIdx), WORD(m_dwCurrEffectFrame));
...
{
	BYTE bDir = BYTE(lpPacketMsg->stDefMsg.wSeries);
	m_wPosX   = lpPacketMsg->stDefMsg.wParam;
	m_wPosY   = lpPacketMsg->stDefMsg.wTag;
...
VOID CHero::OnMagicFire(LPPACKETMSG lpPacketMsg)
{
	...
	bMagicID = BYTE(lpPacketMsg->stDefMsg.wSeries);
	...
	if ( bMagicID == _SKILL_SHOOTLIGHTEN )
	{
		...
		pxElecMagic->CreateMagic(bMagicID, SHORT(m_wPosX), SHORT(m_wPosY), SHORT(ptPos.x), SHORT(ptPos.y), NULL, nMagicTargetID);
...
//	if ( bMagicID == _SKILL_SHOOTLIGHTEN || bMagicID == _SKILL_FIRE )
	...
	{
		BYTE  bDir16	 = (BYTE)g_xGameProc.m_xMap.CalcDirection8(WORD(m_wPosX), WORD(m_wPosY), WORD(ptPos.x), WORD(ptPos.y));
		POINT ptTileGap  = {0, 0};
...
BOOL CMyHero::OnLButtonDown(POINT ptMouse, INT nTargetID, BOOL bIsDead, POINT* lpptTargetPos)
{
	...
	if ( !m_bMotionLock && !m_bInputLock )
	{
		...
		if ( lpptTargetPos ) 
			...
			nLength = (INT)sqrt(float((m_wPosX-ptMouseTilePos.x)*(m_wPosX-ptMouseTilePos.x) + (m_wPosY-ptMouseTilePos.y)*(m_wPosY-ptMouseTilePos.y)));
