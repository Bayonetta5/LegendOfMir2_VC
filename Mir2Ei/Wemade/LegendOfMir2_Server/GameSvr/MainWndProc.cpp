...
UINT WINAPI InitializingServer(LPVOID lpParameter)
{
	...
	ConnectToServer(g_csock, &g_caddr, _IDM_CLIENTSOCK_MSG, NULL, dwIP, nPort, FD_CONNECT|FD_READ|FD_CLOSE);//连接到数据库服务器
	ConnectToServer(g_clsock, &g_claddr, _IDM_LOGSVRSOCK_MSG, NULL, dwIP, 5500, FD_CONNECT|FD_READ|FD_CLOSE);//连接游戏网关
...
LPARAM APIENTRY MainWndProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	switch (nMsg)
	{
		...
		case _IDM_CLIENTSOCK_MSG:
			return OnClientSockMsg(wParam, lParam);//数据库服务器消息
		case _IDM_LOGSVRSOCK_MSG:
			return OnLogSvrSockMsg(wParam, lParam);//登陆服务器消息
		case WM_COMMAND:
