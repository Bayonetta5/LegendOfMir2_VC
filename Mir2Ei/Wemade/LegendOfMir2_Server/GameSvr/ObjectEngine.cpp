...
/*        if WAbil.HP = 0 then begin
            if BoAbilRevival then begin //Àç»ý ´É·ÂÀÌ ÀÖ´Ù.
               if GetTickCount - LatestRevivalTime > 60 * 1000 then begin
                  ...
                  HealthSpellChanged;
                  SysMsg ('¹ÝÁöÀÇ ÈûÀ¸·Î ¸öÀÌ Àç»ýµÇ¾ú½À´Ï´Ù.', 1);
               end;
            end;
            if WAbil.HP = 0 then
               Die;
         ...
         if not guildwarkill then begin //¹®ÆÄÀüÀ¸·Î Á×À½
            if not LastHiter.IsGoodKilling(self) then begin
               ...
               if PkLevel < 1 then //Á×Àº »ç¶÷ÀÌ ÂøÇÑ »ç¶÷
                  if Random(5) = 0 then //»ìÀÎÀ» ÇÏ¸é ¹«±â°¡ ÀúÁÖ¸¦ ¹Þ´Â´Ù.
	...
	if (m_xVisibleObjectList.GetCount())
	{
		...
		while (pListNode)
		{
			if (pVisibleObject = m_xVisibleObjectList.GetData(pListNode))
				pVisibleObject->nVisibleFlag = 0;//ÉèÖÃ²»¿É¼û
				...
				if (pMapCellInfo->m_xpObjectList)
				{
					if (pMapCellInfo->m_xpObjectList->GetCount())
					{
						...
						while (pListNode)
						{
							...
							if (pOSObject)
							{
								if (pOSObject->btType == OS_MOVINGOBJECT)//Èç¹ûÊÇ¿ÉÒÆ¶¯¶ÔÏó
								{
									pCharObject = (CCharObject*)pOSObject->pObject;
									
									if (!pCharObject->m_fInspector && !pCharObject->m_fIsGhost && !pCharObject->m_fHideMode)
										UpdateVisibleObject(pCharObject);//ÕÒµ½ËùÓÐ¿É¼û¶ÔÏó
								}
								else if (pOSObject->btType == OS_ITEMOBJECT)//Èç¹ûÊÇÎïÆ·¶ÔÏó
								{
									if (GetTickCount() - pOSObject->dwAddTime > 60 * 60 * 1000)//Èç¹û¶ÔÏó³¬¹ýÒ»¸öÐ¡Ê±Ö±½ÓÒÆ³ý
									{
										delete pOSObject;
										pOSObject = NULL;
										...
										UpdateVisibleItem(x, y, (CMapItem*)pOSObject->pObject); //ÏÔÊ¾ÎïÆ·
								}
								else if (pOSObject->btType == OS_EVENTOBJECT)//ÊÂ¼þ¶ÔÏó
								{
...
BOOL CCharObject::HitXY(WORD wIdent, int nX, int nY, int nDir, int nHitStyle)
{
	...
	if (m_nHitTimeOverCount < 2)
	{
		if (m_nCurrX == nX && m_nCurrY == nY)
		{
			...
			if (m_pUserInfo->m_lpTMagicPowerHitSkill)		// ¿¹µµ °Ë¹ý
			{
   ...
   for i:=0 to 12 do begin
      ...
      if cret <> nil then begin
         ...
         if IsProperTarget (cret) then begin
            if cret.AntiMagic <= Random(10) then begin  //¸¶¹ý È¸ÇÇ°¡ ÀÖÀ½
		...
		if (GetNextPosition(nDir, 1, nX, nY))
		{
			...
			if (pTargetObject && (nDamage > 0))
			{
				if (IsProperTarget(pTargetObject))
				{
					...
					sprintf(szMsg, "%s ¹Ý¿ù ¸ÂÀ½ - PW:%d HP:%d", pTargetObject->m_szName, nDamage, pTargetObject->m_WAbility.HP);
					SysMsg(szMsg, 0);
					// Debug Code
#endif
					return TRUE;
